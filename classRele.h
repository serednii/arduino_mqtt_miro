...
{ //читаємо address з памяті ЕЕПРОМ
  ...
  for (int i = 0; i < NUMBER_RELE; i++)
  {
    ...
    //    Serial.print("void readEepromNameRele()    ");
    //    Serial.print(i);
...
void manual_vklOtkl(int onOff, int numRele ) {
  if (onOff == 0) { //Виключаємо реле
    ...
    //    releControl[numRele].onOffManual = 0;
    //    byte temp = EEPROM.read(START_SECTION_RELE_EEPROM_UPR + numRele);
    //    temp &= ~0b00000010;//reset bit 1
    //    EEPROM.write(START_SECTION_RELE_EEPROM_UPR + numRele, temp);
    //    comitEprom();
    ...
    //    releControl[numRele].onOffManual = 1;
